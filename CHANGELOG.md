...
- **Schedule animation — classify domain rank before building a convex hull** (issue #142,
  ...
  to detect a degenerate (coplanar/collinear) domain, but QuickHull needs *four*
  affinely-independent points and on a rank-reduced set either throws or returns a garbage sliver
  — and 15 shipped schedules have 1-D/2-D domains (`dot`/`asum`/`nrm2` are lines,
  `gemv`/`axpy`/`trmv`/… are planes). Now the **affine rank** of each domain's point set is
  computed first: only a rank-3 domain builds a solid hull; a rank-0/1/2 domain (and any rank-3
  hull that still fails) falls back to a bounding-box outline — a single-point (rank-0) domain,
  whose box is zero-size, is marked with a small sphere so it stays visible. Adds `affineRank`
  (exported, pure) and a `test/hull-rank.mjs` unit test over two-point, collinear, planar, and
  full-3-D fixtures (wired into `npm test`).
...
- **Schedule animation — convex-hull domains of computation** (issue #142): to visualize *large*
  ...
  domain (a face or line) falls back to a bounding-box outline. Opt an embed into hull-first with a
  bare `data-hull` attribute (single-view and `schedule-compare` both); the *Systolic SURE
  transforms* page's two 6-variable animations now open in hull view. Suppressed in tile mode.
  (`docs-site/src/components/schedule-anim.js`, `src/styles/custom.css`.)
...
- **Schedule animation — offline PNG→video path + landing-page clip** (issue #142, Phase 3):
  ...
  `.gif` — the same offline path cortex uses. It serves the built `dist/` via `astro preview`,
  drives the viewer through a new global capture hook (`window.__scheduleViewers[i].setFrame(f)` /
  `.frameCount`, with each container tagged `data-sv-index`), reads each frame's pixels with
  `canvas.toDataURL` (renderer `preserveDrawingBuffer:true`; avoids the Playwright-screenshot
  deadlock against the page's `requestAnimationFrame` loops, and captures the canvas only), and
  encodes with ffmpeg. Playwright is a devDependency; its browsers and ffmpeg stay out-of-band, so
  the Pages build stays toolchain-free. The first clip (`public/videos/matmul-linear.mp4`) is
  committed and embedded on the **landing page** — the matmul linear schedule with its τ-plane
  sweep. Flags cover the target page, viewer index, selector, output/codec, fps, size, and loops.
