// syr2k_sure.cpp
//
// BLAS Level-3 symmetric rank-2k update as a System of Uniform Recurrence Equations
// (issue #39, docs/SURE/syr2k.sure):
//
//   C(i,j) := beta*C(i,j) + alpha*sum_k ( A(i,k)B(j,k) + B(i,k)A(j,k) )   on j<=i,
//
// i.e. C := beta*C + alpha*(A B^T + B A^T) over the triangular output. syr2k is syrk
// with two operands: four operand streams (A(i,k)/B(i,k) along +j, A(j,k)/B(j,k) on
// the super-diagonal along +i) feed the two-term product. alpha is folded into the
// reduction; beta*C_in joins the completed sum in a terminal-face epilogue on k=K-1.
//
...
//   - the numeric result beta*Cin + alpha*(A B^T + B A^T) (lower) against a reference
...
int main() {
    bool ok = true;
    try {
        SureSpec spec = parseSureFile(std::string(SURE_DOCS_DIR) + "/syr2k.sure");

        // ---- parsed structure ----
        ok &= (spec.indexNames == std::vector<std::string>{ "i", "j", "k" });
        ok &= (spec.tau == std::vector<int>{ 2, 1, 1 });
        ok &= (spec.inputs.size() == 8);   // A->ai, B->bi, A->aj, B->bj, Alpha, Beta, Cin, seed
        ok &= (spec.outputs.size() == 1);
        std::cout << "parsed structure (indices, tau, 8 inputs, 1 output): "
                  << (ok ? "PASS" : "FAIL") << "\n";

        if (spec.outputs.empty() || spec.inputs.empty())
            throw std::runtime_error("syr2k: parse produced no input/output confluence");

        // ---- derived confluence orientation: outward face normals ----
        // A/B +j taps on j=-1 (0,-1,0); A/B super-diagonal +i taps on i-j=-1 (-1,1,0);
        // alpha/beta/Cin/seed on k=-1; the result exits on k=K-1.
        bool nok = true;
        for (const auto& in : spec.inputs) {
            if (in.tensor == "A" || in.tensor == "B") {
                nok &= (in.normal == std::vector<int>{ 0, -1, 0 } || in.normal == std::vector<int>{ -1, 1, 0 });
            } else {
                nok &= (in.normal == std::vector<int>{ 0, 0, -1 });   // Alpha, Beta, Cin, seed
            }
        }
        nok &= (spec.outputs.front().normal == std::vector<int>{ 0, 0, 1 });
        std::cout << "derived face normals (incl. super-diagonal feeds): " << (nok ? "PASS" : "FAIL") << "\n";
        ok &= nok;

        // ---- numeric result: beta*Cin + alpha*(A B^T + B A^T) on j<=i ----
        const size_t N = 3, K = 2;
        const double A[3][2] = { { 1, 2 }, { 3, 4 }, { 5, 6 } };
        const double B[3][2] = { { 1, 0 }, { 0, 1 }, { 1, 1 } };
        ...
        for (size_t i = 0; i < N; ++i)
            for (size_t j = 0; j <= i; ++j) {
                double s = 0;
                for (size_t k = 0; k < K; ++k) s += A[i][k] * B[j][k] + B[i][k] * A[j][k];
        ...
        std::cout << "parsed syr2k == beta*Cin + alpha*(A B^T + B A^T) (lower): " << (numok ? "PASS" : "FAIL") << "\n";
        ok &= numok;
        ...
        // ---- the box-default tau=[1,1,1] must be rejected (super-diagonal feeds) ----
        bool fok = false;
