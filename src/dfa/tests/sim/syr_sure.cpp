// syr_sure.cpp
//
// BLAS Level-2 symmetric rank-1 update as a System of Uniform Recurrence Equations
// (issue #35, docs/SURE/syr.sure):
//
//   A(i,j) := A(i,j) + alpha * x(i) * x(j)   on the triangular domain j <= i.
//
// syr is ger with y = x (one vector on both axes) and the output restricted to one
// triangle (A + alpha x x^T is symmetric). A enters and exits on the (i,j) face of a
// depth-2 feed/drain axis; the single vector x feeds both axes per cell; the rank-1
// term is added once (alpha injected only on the k=-1 halo). It is the SPD primitive
...
//   - the numeric result A + alpha x x^T over the lower triangle against a reference
...
int main() {
    bool ok = true;
    try {
        SureSpec spec = parseSureFile(std::string(SURE_DOCS_DIR) + "/syr.sure");

        // ---- parsed structure ----
        ok &= (spec.indexNames == std::vector<std::string>{ "i", "j", "k" });
        ok &= (spec.tau == std::vector<int>{ 1, 1, 1 });
        ok &= (spec.inputs.size() == 4);   // A (seed r), Alpha, X->xx, X->yy
        ok &= (spec.outputs.size() == 1);
        std::cout << "parsed structure (indices, tau, 4 inputs, 1 output): "
                  << (ok ? "PASS" : "FAIL") << "\n";

        if (spec.outputs.empty() || spec.inputs.empty())
            throw std::runtime_error("syr: parse produced no input/output confluence");

        // ---- derived confluence orientation: outward face normals ----
        // A, alpha and both x feeds enter on the (i,j) face k=-1; the updated lower
        // triangle drains on the k=1 face.
        bool nok = true;
        ...
        // ---- numeric result: A(i,j) + alpha x(i) x(j) on j <= i (mirrors the spec) ----
        const size_t N = 3;
        ...
        std::cout << "parsed syr == A + alpha x x^T (lower): " << (numok ? "PASS" : "FAIL") << "\n";
        ok &= numok;
