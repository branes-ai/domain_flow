...
public:
    ...
    std::vector<double> solve(const std::vector<std::vector<double>>& A,
        ...
        for (int i = 0; i < m; ++i) {
            ...
            for (int j = 0; j < n; ++j) {
                tableau_[i][j] = sign * A[i][j];
            }
            tableau_[i][n + i] = sign; // Slack variable
            tableau_[i][total_vars] = sign * b[i];
            if (needs_artificial[i]) {
                tableau_[i][artificial_idx] = 1.0; // Artificial variable
                basis_[i] = artificial_idx++;
            } else {
                basis_[i] = n + i;
            }
        }
        ...
        if (num_artificial > 0) {
            ...
            if (std::abs(tableau_[m][total_vars]) > 1e-6) {
                throw std::runtime_error("LP is infeasible");
            }
...
private:
    ...
    bool is_inside_hull(const std::vector<int>& point) const {
        for (const auto& plane : planes_) {
            ...
            if (value > plane.b + 1e-6) { // Small numerical tolerance
                return false;
            }
        }
        return true;
    ...
    void compute_bounding_box() {
        ...
        // Validate bounds
        for (int d = 0; d < dimension_; ++d) {
            if (bounds_min_[d] > bounds_max_[d]) {
