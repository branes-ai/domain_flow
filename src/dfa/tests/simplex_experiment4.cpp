...
public:
    ...
    std::vector<double> solve(const std::vector<std::vector<double>>& A,
		...
		// for every constraint, if b[i] < 0, we need to add an artificial variable
		// and negate the constraint and rhs
		...
		// Phase 1: Minimize sum of artificial variables: min Z = sum of artificial variables
        if (num_artificial > 0) {
...
0       -1      1       1
0       -1      0       -1
Dimension 0: min = 0, max = 1
Integer lattice points in 1D convex hull:
(1)
Total points: 1
 */