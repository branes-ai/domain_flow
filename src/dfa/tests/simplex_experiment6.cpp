...
public:
    ...
    std::vector<double> solve(const std::vector<std::vector<double>>& A,
        ...
        for (int i = 0; i < m; ++i) {
            for (int j = 0; j < n; ++j) {
                tableau_[i][j] = needs_artificial[i] ? -A[i][j] : A[i][j]; // Correct coefficient
            }
        ...
        if (num_artificial > 0) {
            // Initialize Phase 1 objective
            for (int j = 0; j <= total_vars; ++j) {
                tableau_[m][j] = 0.0;
            }
            for (int i = 0; i < m; ++i) {
                if (needs_artificial[i]) {
                    tableau_[m][basis_[i]] = -1.0; // Minimize artificial variables
                }
            }

            // Debug: Print initial Phase 1 tableau
            std::cout << "Phase 1 Initial Tableau:\n";
            for (int i = 0; i <= m; ++i) {
                for (int j = 0; j <= total_vars; ++j) {
            ...
            // Debug: Print final Phase 1 tableau
            std::cout << "Phase 1 Final Tableau:\n";
            for (int i = 0; i <= m; ++i) {
                for (int j = 0; j <= total_vars; ++j) {
                    std::cout << tableau_[i][j] << "\t";
                }
            ...
            // Prepare for Phase 2: Remove artificial variables
            std::vector<std::vector<double>> new_tableau(m + 1, std::vector<double>(n + m + 1));
            for (int i = 0; i <= m; ++i) {
                for (int j = 0; j < n + m; ++j) {
                    new_tableau[i][j] = tableau_[i][j];
                }
                new_tableau[i][n + m] = tableau_[i][total_vars];
            }
            tableau_ = new_tableau;
            total_vars = n + m;
            ...
            for (int i = 0; i < m; ++i) {
                if (basis_[i] >= n + m) {
                    for (int j = 0; j < n + m; ++j) {
                        if (std::abs(tableau_[i][j]) > 1e-10 && j != n + i) {
                            basis_[i] = j;
                            double pivot = tableau_[i][j];
                            for (int k = 0; k <= total_vars; ++k) {
                                tableau_[i][k] /= pivot;
                            }
                            for (int r = 0; r <= m; ++r) {
                                if (r != i) {
                                    double factor = tableau_[r][j];
                                    for (int k = 0; k <= total_vars; ++k) {
                                        tableau_[r][k] -= factor * tableau_[i][k];
                                    }
                                }
                            }
                            break;
        ...
        // Debug: Print Phase 2 initial tableau
        std::cout << "Phase 2 Initial Tableau:\n";
        for (int i = 0; i <= m; ++i) {
            for (int j = 0; j <= total_vars; ++j) {
                std::cout << tableau_[i][j] << "\t";
            }
    ...
    void compute_bounding_box() {
        ...
        for (int d = 0; d < dimension_; ++d) {
            ...
            try {
                ...
                std::cerr << "Max LP failed for dimension " << d << ": " << e.what() << "\n";
            ...
            try {
                ...
                std::cerr << "Min LP failed for dimension " << d << ": " << e.what() << "\n";
        ...
        for (int d = 0; d < dimension_; ++d) {
            if (bounds_min_[d] > bounds_max_[d] + 1e-10) {
                throw std::runtime_error("Invalid bounding box: min > max for dimension " + std::to_string(d));
            }
        }
    }
